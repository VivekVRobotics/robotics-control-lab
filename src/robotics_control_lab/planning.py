...
def edge_collision_free(a: np.ndarray, b: np.ndarray, obstacles: list[CircleObstacle], step: float = 0.01) -> bool:
    ...
    return all(not any(ob.contains(a + t * (b - a)) for ob in obstacles) for t in np.linspace(0.0, 1.0, samples + 1))
...
def rrt(start, goal, bounds, obstacles, *, iterations=2000, step_size=0.08, goal_bias=0.1, seed=0):
    ...
    start = np.asarray(start, dtype=float).reshape(2); goal = np.asarray(goal, dtype=float).reshape(2)
    ...
    for _ in range(iterations):
        ...
        direction = sample - nodes[nearest]; norm = float(np.linalg.norm(direction))
        if norm < 1e-12: continue
        new = nodes[nearest] + step_size * direction / norm if norm > step_size else sample
        if not all(bounds[i][0] <= new[i] <= bounds[i][1] for i in range(2)): continue
        if not edge_collision_free(nodes[nearest], new, obstacles): continue
...
def rrt_star(start, goal, bounds, obstacles, *, iterations=3000, step_size=0.08, neighbor_radius=0.2, goal_bias=0.05, seed=0):
    ...
    if iterations <= 0 or step_size <= 0 or neighbor_radius <= 0: raise ValueError("invalid RRT* parameters")
    rng = np.random.default_rng(seed)
    start = np.asarray(start, dtype=float).reshape(2); goal = np.asarray(goal, dtype=float).reshape(2)
    nodes = [start]; parents = [-1]; costs = [0.0]; best_goal = None
    for _ in range(iterations):
        sample = goal if rng.random() < goal_bias else np.array([rng.uniform(*bounds[0]), rng.uniform(*bounds[1])])
        nearest = int(np.argmin([np.linalg.norm(n - sample) for n in nodes]))
        direction = sample - nodes[nearest]; norm = float(np.linalg.norm(direction))
        ...
        if not all(bounds[i][0] <= new[i] <= bounds[i][1] for i in range(2)): continue
        if not edge_collision_free(nodes[nearest], new, obstacles): continue
        near = [i for i, n in enumerate(nodes) if np.linalg.norm(n - new) <= neighbor_radius and edge_collision_free(n, new, obstacles)]
        parent = min(near, key=lambda i: costs[i] + np.linalg.norm(nodes[i] - new)) if near else nearest
        new_cost = costs[parent] + np.linalg.norm(nodes[parent] - new)
        nodes.append(new); parents.append(parent); costs.append(new_cost)
        new_idx = len(nodes) - 1
        for i in near:
            candidate = new_cost + np.linalg.norm(nodes[i] - new)
            if candidate + 1e-12 < costs[i]:
                parents[i] = new_idx; costs[i] = candidate
        if np.linalg.norm(new - goal) <= step_size and edge_collision_free(new, goal, obstacles):
            goal_cost = new_cost + np.linalg.norm(goal - new)
            if best_goal is None or goal_cost < best_goal[0]:
                nodes.append(goal); parents.append(new_idx); costs.append(goal_cost); best_goal = (goal_cost, len(nodes) - 1)
...
def astar(grid: np.ndarray, start: tuple[int, int], goal: tuple[int, int]) -> list[tuple[int, int]] | None:
    ...
    if grid.ndim != 2: raise ValueError("grid must be 2D")
    if grid[start] != 0 or grid[goal] != 0: return None
    frontier = [(0.0, start)]; parent = {start: None}; cost = {start: 0.0}
    ...
    while frontier:
        ...
        for dr, dc in neighbors:
            ...
            if not (0 <= nxt[0] < grid.shape[0] and 0 <= nxt[1] < grid.shape[1]) or grid[nxt] != 0: continue
            new_cost = cost[current] + math.hypot(dr, dc)
            if nxt not in cost or new_cost < cost[nxt]:
                cost[nxt] = new_cost; h = math.hypot(goal[0] - nxt[0], goal[1] - nxt[1])
